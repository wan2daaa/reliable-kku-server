...
class AdminOrderServiceTest extends BaseServiceTest {
    ...
    void 일별_한시간별_주문의합을_반환한다() throws Exception {
        ...
        when(adminOrderRepository.findByEachTimeSumOfOrderPriceByDateBetween(any(), any(),
                any())).thenReturn(
...
//    LocalDate expect = LocalDate.from(today.plusMonths(1)..minusDays(1));
    ...
    void 매출관리_달력_데이터를_반환한다() throws Exception {
        ...
        Order order1 = Order.builder().id(1L).orderPrice(3500).createdDate(LocalDate.of(2023, 11, 2))
                .orderStatus(OrderStatus.FINISH).build();
        Order order2 = Order.builder().id(2L).orderPrice(4500).createdDate(LocalDate.of(2023, 11, 3))
                .orderStatus(OrderStatus.FINISH).build();
        Order order3 = Order.builder().id(3L).orderPrice(4500).createdDate(LocalDate.of(2023, 11, 4))
                .orderStatus(OrderStatus.CANCELED).build();
        Order order8 = Order.builder().id(8L).orderPrice(4500).createdDate(LocalDate.of(2023, 11, 4))
                .orderStatus(OrderStatus.FINISH).build();
        Order order4 = Order.builder().id(4L).orderPrice(5500).createdDate(LocalDate.of(2023, 10, 2))
                .orderStatus(OrderStatus.FINISH).build();
        Order order5 = Order.builder().id(5L).orderPrice(6500).createdDate(LocalDate.of(2023, 10, 3))
                .orderStatus(OrderStatus.FINISH).build();
        Order order6 = Order.builder().id(6L).orderPrice(2500).createdDate(LocalDate.of(2023, 10, 4))
                .orderStatus(OrderStatus.FINISH).build();
        Order order7 = Order.builder().id(7L).orderPrice(2500).createdDate(LocalDate.of(2023, 10, 4))
                .orderStatus(OrderStatus.CANCELED).build();
        ...
        when(adminOrderRepository.findCalendarMonthDataByStartDateAndLastDateBetween(lastMonthFirstDay,
                lastMonthLastDay)).thenReturn(10000);

        LocalDate thisMonthFirstDay = date.with(firstDayOfMonth());
        LocalDate thisMonthLastDay = date.with(lastDayOfMonth());

        when(adminOrderRepository.findCalendarMonthDataByStartDateAndLastDateBetween(thisMonthFirstDay,
                thisMonthLastDay)).thenReturn(100000);
        when(adminOrderRepository.findTotalRefundSalesOfMonthByStartDateAndLastDateBetween(
                thisMonthFirstDay, thisMonthLastDay)).thenReturn(3000);
    ...
    void 대기_접수_완료의_주문_개수를_반환한다() {
        ...
        Order.builder().createdDate(LocalDate.of(2023, 11, 11)).orderStatus(OrderStatus.COOKING)
                .build();
        Order.builder().createdDate(LocalDate.of(2023, 11, 11)).orderStatus(OrderStatus.PICKUP).build();
        Order.builder().createdDate(LocalDate.of(2023, 11, 11)).orderStatus(OrderStatus.CANCELED)
                .build();
        Order.builder().createdDate(LocalDate.of(2023, 11, 11)).orderStatus(OrderStatus.FINISH).build();
        Order.builder().createdDate(LocalDate.of(2023, 11, 11)).orderStatus(OrderStatus.NOT_TAKE)
                .build();
        LocalDate currentDate = LocalDate.of(2023, 11, 11);
        when(orderRepository.findOrderCountByOrderStatusAndDate(List.of(OrderStatus.WAIT),
                currentDate)).thenReturn(1L);
        when(orderRepository.findOrderCountByOrderStatusAndDate(
                List.of(OrderStatus.COOKING, OrderStatus.PICKUP), currentDate)).thenReturn(2L);
        when(orderRepository.findOrderCountByOrderStatusAndDate(
                List.of(OrderStatus.FINISH, OrderStatus.CANCELED, OrderStatus.NOT_TAKE),
                currentDate)).thenReturn(3L);
        //when
